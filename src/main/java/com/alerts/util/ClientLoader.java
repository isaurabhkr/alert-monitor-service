    ...
    public static List<ClientConfig> loadClientConfigs(String propsFilePath) throws Exception {
        ...
        for (String key : props.stringPropertyNames()) {
            ...
            if (key.startsWith(CLIENT_PROP_PREFIX)) {
                ...
                int alertCount = Integer.parseInt(props.getProperty(key + "."+client+"."+eventType+ALERT_COUNT_PROP_PREFIX));
                int alertWindowSize = Integer.parseInt(props.getProperty(key +"."+client+ "."+eventType+ ALERT_WINDOW_SIZE_PROP_PREFIX));
                ...
                for (int i = 0; ; i++) {
                    ...
                    String messageKey = key + "."+client+ "."+eventType+ DISPATCH_STRATEGY_MESSAGE_PROP_PREFIX + "[" + i + "]";
                    String subjectKey = key + "."+client+ "."+eventType+ DISPATCH_STRATEGY_SUBJECT_PROP_PREFIX + "[" + i + "]";
                ...
                clientConfigs.add(new ClientConfig(client, eventType, alertType, alertCount, alertWindowSize, dispatchStrategies));
            }
        }

        return clientConfigs.stream().distinct().collect(
                Collectors.toList());
    }
}